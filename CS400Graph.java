...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
     ...
     *         larger distance than the other, and the comparison of end vertex data in string form
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    if (start == null | end == null) {
      throw new NoSuchElementException("Either start or end cannot be null.");
    }
    ...
    if(vertices.get(end) == startVert) {
      return new Path(startVert);
    }
    LinkedList<Vertex> visited = new LinkedList<>();// keep track of the visited node
    paths.add(new Path(startVert));// store the first original path to the priority queue
    while (!paths.isEmpty()) {
      Path currentPath = paths.poll();// retrieve the shortest path in the priority queue
      Vertex currentVert = currentPath.end;
      // For every edges leaving the current node, extend them and add to paths
      for (Edge e : currentVert.edgesLeaving) {
        Vertex endVert = e.target;
        if (visited.contains(endVert)) {
          continue;
        }
        Path newPath = new Path(currentPath);
        newPath.extend(e);
        if (endVert.data.equals(end)) {
          return newPath;
        }
        paths.add(newPath);
